...
lazy val ForStmt: Parser[List[Token], Stmt] =
  (p("for") ~ VarParser ~ p(":=") ~ AExp ~ p("upto") ~ AExp ~ p("do") ~ Block).map {
    ...
    case _ => throw new RuntimeException("invalid for stmt")
  }
...
lazy val Stmt: Parser[List[Token], Stmt] =
  (p("write") ~ StringParser).map{ case (x, y) => WriteString(y)}
    ...
    || (p("write") ~ p("(") ~ VarParser ~ p(")")).map {
      case (((x, y), Var(z)), u) => WriteId(z)
      case _ => throw new RuntimeException("invalid write stmt")
    }
    || (p("read") ~ VarParser).map{ case (x, Var(y)) => Read(y)}
    || (VarParser ~ p(":=") ~ AExp).map{
      case ((Var(x), y), z) => Assign(x, z)
      case _ => throw new RuntimeException("invalid assign stmt")
    }
    || p("skip").map{ x => Skip}
...
// Bytecode Compiler
// ==========
var class_name: String = "test/test"
...
def compile_stmt(stmt: Stmt, env: Env): (String, Env) = stmt match {
  ...
  case WriteString(s) =>
    var instr = s"  ldc ${s}\n"
    instr += s"  invokestatic ${class_name}/writes(Ljava/lang/String;)V\n"
    (instr, env)
  case WriteId(var_name) => find_var(var_name, env) match {
    case Some(i) =>
      var instr = s"  iload ${i}\n"
      instr += s"  invokestatic ${class_name}/write(I)V\n"
      (instr, env)
    case None => throw new RuntimeException("no var in env: " + var_name)
  }
  case Read(var_name) =>
    var instr = s"  invokestatic ${class_name}/read()I\n"
...
def compile(bl: Block, file: String) : String = {
  class_name = file
  var asm = ""
  asm += (
    s"""
...
def single_test(file: String): Unit = {
  val contents = os.read(os.pwd / "examples" / (file + ".while"))
  println(s"Lex $file: ")
  ...
  println(ast)
//  println(compile(ast, file))
  os.write(os.pwd / (file + ".j"), compile(ast, file))
}

@main
// Test all example files
def test(): Unit = {
  val examples = List("loops", "primes")
